...
func (suite *IntegrationTestSuite) TestTotalSupply() {
	...
	testcases := []struct {
	...
	}{
		{
			...
			types.NewGenesisState(defaultGenesis.Params, balances, sdk.NewCoins(sdk.NewCoin("wrongcoin", sdk.NewInt(1))), nil, nil, defaultGenesis.DenomMetadata),
			nil, true, "genesis supply is incorrect, expected 1wrongcoin, got 21barcoin,11foocoin",
		},
		{
			"calculation matches genesis Supply field",
			types.NewGenesisState(defaultGenesis.Params, balances, totalSupply, nil, nil, defaultGenesis.DenomMetadata),
			totalSupply, false, "",
		},
		{
			"calculation is correct, empty genesis Supply field",
			types.NewGenesisState(defaultGenesis.Params, balances, nil,nil, nil, defaultGenesis.DenomMetadata),
			totalSupply, false, "",
		},
	}
